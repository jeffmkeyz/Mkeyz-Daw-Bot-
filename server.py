...
def _get_vps(cur, tg_id):
    ...
    if not row:
        cur.execute("INSERT INTO vps_state (tg_id, updated_at) VALUES (?,?)",
                    (tg_id, int(time.time())))
        return {"cpu":1,"ram":1024,"storage":20,"bandwidth":1,"vps_name":"VPS-SIM-01"}
    return dict(row)
...
def api_vps_upgrade():
    ...
    _ensure_vps_tables(cur); con.commit()
    vps = _get_vps(cur, tg_id)
    new_val = min(LIMITS[rtype], vps[rtype] + (float(amount) if rtype == "cpu" else int(amount)))
