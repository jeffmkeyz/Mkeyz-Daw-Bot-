...
BANNER_URL = "https://i.ibb.co/FbwPWzRC/Picsart-26-04-26-00-55-47-981.jpg"
...
MKEYZ_REWARDS = {
    ...
    "listener_panel":  20,   # Usar Panel de Oyentes
}
...
def kb_sub_tools():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎹  Listener Panel", callback_data="sec_panel")],
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "sec_scale":
        ...
        if game_url:
            await edit(q,
                ...
                "🎧 Diferenciación entre tus plays y otros artistas\n"
    ...
    # ── Tarjeta de Artista ─────────────────────────────────
    if d == "sec_card":
        game_url = (GAME_URL.rstrip("/") + "/card") if GAME_URL else None
        if game_url:
            await edit(q,
                "🃏 *Tarjeta de Artista*\n\n"
                "Genera tu tarjeta de presentación personalizada\n"
                "para compartir en Instagram, TikTok y redes.\n\n"
                "✅ Se autocompleta con tu perfil de Zona Artistas\n"
                "✅ Personalizable al 100%\n"
                "⬇️ Descargable como PNG\n"
                "📤 Compartible directamente",
                InlineKeyboardMarkup([
                    [InlineKeyboardButton("🃏 Crear mi tarjeta", web_app=WebAppInfo(url=game_url))],
                    [InlineKeyboardButton("← Jeff Mkeyz", callback_data="sub_info")],
                ]))
        else:
            await edit(q, "🃏 No disponible.", kb_back())
